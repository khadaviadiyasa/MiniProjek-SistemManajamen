<?php

$Product = [
   [ 
    "ID" => "S001",
    "Nama" => "AC",
    "Kategori" => "Pendingin dan pengatur udara",
    "Harga" => 2300000,
    "Stok" => 10,
    "Deskripsi" => "pendingin udara dalam ruangan yang dapat disesuaikan"
   ],

   [
    "ID" => "S002",
    "Nama" => "Kulkas",
    "Kategori" => "Penyimpanan dan Pengolahan makanan",
    "Harga" => 3000000,
    "Stok" => 13,
    "Deskripsi" => "Dilengkapi dengan pengaturan suhu, rak yang dapat disesuaikan, dan ruang penyimpanan yang luas, kulkas ini cocok digunakan untuk kebutuhan rumah tangga."
   ],

   [ 
    "ID" => "S003",
    "Nama" => "Rice cooker",
    "Kategori" => "Penyimpanan dan Pengolahan makanan",
    "Harga" => 300000,
    "Stok" => 8,
    "Deskripsi" => "Rice Cooker Multifungsi adalah perangkat elektronik rumah tangga yang dirancang untuk memasak dan menjaga nasi tetap hangat secara praktis."
   ],

   [ 
    "ID" => "S004",
    "Nama" => "Mesin cuci",
    "Kategori" => "Kebersihan dan Perawatan Pakaian",
    "Harga" => 750000,
    "Stok" => 10,
    "Deskripsi" => "Mesin ini dapat melakukan proses mencuci, membilas, dan mengeringkan pakaian secara otomatis sehingga menghemat waktu dan tenaga."
   ],

   [ 
    "ID" => "S005",
    "Nama" => "penyedot debu (vacuum cleaner)",
    "Kategori" => "Kebersihan dan Perawatan Pakaian",
    "Harga" => 250000,
    "Stok" => 15,
    "Deskripsi" => "pembersih yang dirancang untuk membantu membersihkan debu, kotoran, dan partikel kecil dari berbagai permukaan seperti lantai, karpet, sofa, dan sudut ruangan."
   ]


];
?>